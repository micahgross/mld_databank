# -*- coding: utf-8 -*-
"""
Created on Fri Dec 22 14:58:10 2023
...
if upload_file is not None:
    ...
    n_reps = int(len(csv_data.columns) / 6)
    csv_data.columns = [
    ...
    auf_fig = plt.figure()
    plt.title('auf')
    for col in csv_data.columns[4:][::6]:
        csv_data[col].plot(label=col)
    plt.legend() 
    st.pyplot(auf_fig)
    
    ab_fig = plt.figure()
    plt.title('ab')
    for col in csv_data.columns[5:][::6]:
        csv_data[col].plot(label=col)
    plt.legend()    
    st.pyplot(ab_fig)

    st.markdown(
        generate_excel(csv_data),
